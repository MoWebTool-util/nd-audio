...
var AudioHtml5 = Base.extend({
  ...
  initialize: function (config) {
    var _this = this;
    AudioHtml5.superclass.initialize.call(this, config);
    this.element = createPlayer(this.get('src'));
    this.init = true;
    this.loaded = false;
    $(this.element).bind('timeupdate', function () { // 播放进度
      _this.trigger('timeupdate', _this.element.currentTime, _this.element.duration);
    }).bind('ended', function () { // 结束
      resetStatus.call(_this);
      _this.trigger('ended');
    }).bind('error', function () { // 错误
      if (_this.get('src')) {
        _this.trigger('error');
      }
